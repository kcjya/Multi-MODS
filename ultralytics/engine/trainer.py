...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.part_i = 0
        self.mem, self.box_loss, self.cls_loss , self.dfl_loss = [], [], [], []
        self.index = 0
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                llist = losses.tolist()
                self.part_i += 1
                self.train_x.append(self.part_i)
                self.mem.append(float(mem.split("G")[0]))
                self.box_loss.append(llist[0])
                self.cls_loss.append(llist[1])
                self.dfl_loss.append(llist[2])
                self.train_y = [self.mem, self.box_loss, self.cls_loss, self.dfl_loss]
                self.run_callbacks("dtrain")
